...
export default function Dashboard() {
  ...
  const escalationIndex = useMemo(() => {
    ...
    return computeEscalationIndex(mapped);
  }, [articles]);
  ...
  const eiLabel = escalationIndex >= 7 ? 'Critical' : escalationIndex >= 4 ? 'Elevated' : 'Moderate';
  ...
  return (
    <div className="max-w-[1800px] mx-auto px-3 sm:px-4 py-3 sm:py-4">
      <h1 className="text-[16px] sm:text-[18px] font-extrabold text-[#111827] tracking-[-0.5px] mb-4">War Dashboard</h1>

      {/* KPI Row */}
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-2 sm:gap-3 mb-4">
        {isLoading ? Array.from({ length: 4 }).map((_, i) => <MetricSkeleton key={i} />) : (<>
          <div className="card p-3 sm:p-4">
            <SectionHeader>Escalation Index</SectionHeader>
            <div className="text-[22px] sm:text-[28px] font-bold font-mono leading-none" style={{ color: eiColor }}>{escalationIndex}/10</div>
            <div className="text-[10px] font-mono mt-1" style={{ color: eiColor }}>{eiLabel}</div>
          </div>
          <div className="card p-3 sm:p-4">
            <SectionHeader>Avg Sentiment</SectionHeader>
            <div className="text-[22px] sm:text-[28px] font-bold font-mono leading-none" style={{ color: sentimentColor }}>{avgSentiment.toFixed(2)}</div>
            <div className="text-[10px] font-mono text-[#9CA3AF] mt-1">comparative score</div>
          </div>
          <div className="card p-3 sm:p-4">
            <SectionHeader>Sources Reporting</SectionHeader>
            <div className="text-[22px] sm:text-[28px] font-bold font-mono leading-none text-[#111827]">{uniqueSources}</div>
          </div>
          <div className="card p-3 sm:p-4">
            <SectionHeader>Countries Mentioned</SectionHeader>
            <div className="text-[22px] sm:text-[28px] font-bold font-mono leading-none text-[#111827]">{countriesMentioned}</div>
          </div>
        </>)}
      </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-3 mb-4">
        {timeline.isLoading ? <ChartSkeleton /> : (
          <div className="card p-4">
            <SectionHeader>Coverage Volume / Hour</SectionHeader>
        ...
        <div className="card p-4">
          ...
          <div className="space-y-4">
            {(['person', 'org', 'location'] as const).map((type) => {
              ...
              return (<div key={type}><div className="section-header mb-2" style={{ fontSize: '9px' }}>{type === 'person' ? 'People' : type === 'org' ? 'Organizations' : 'Locations'}</div><div className="flex flex-wrap gap-1.5">{items.map((e) => (<span key={e.name} className="bg-gray-100 text-gray-700 px-2 py-0.5 rounded-[3px] text-[11px]">{e.name} <span className="text-[#9CA3AF]">({e.count})</span></span>))}</div></div>);
            })}
          </div>
        </div>
      </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-3 mb-4">
        ...
        <div className="card p-4">
          ...
          <div className="text-[22px] sm:text-[28px] font-bold font-mono leading-none mb-1" style={{ color: eiColor }}>{escalationIndex} / 10</div>
          <div className="text-[12px]" style={{ color: eiColor }}>{eiLabel.toUpperCase()} — Based on live data analysis</div>
          <div className="text-[10px] font-mono text-[#9CA3AF] mt-2">Sentiment + keyword density + source breadth</div>
        </div>
      </div>
        ...
        <div className="relative mt-4"><div className="absolute top-3 left-0 right-0 h-[2px] bg-[#E5E7EB]" />
          <div className="flex overflow-x-auto gap-0 pb-4">
            {timelineEvents.map((ev, i) => (
              <div key={i} className="flex flex-col items-center min-w-[120px] sm:min-w-[130px] px-1 relative">
